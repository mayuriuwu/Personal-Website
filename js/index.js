...
function loadContent(consoleOpen) {
  if (consoleOpen) {
    ...
    addMessage("wanna collaborate on a project?");
    addMessage("just message me and we'll find a time and place to talk!");
    const glitch = addMessage("ericzhu105@gmail.com");
...
function convertPage(consoleOpen = true) {
  loadCss(consoleOpen);
  document.title = consoleOpen ? "🏆" : "Eric Zhu";
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  document.querySelector("#title-name").addEventListener(
    "keydown",
    function (e) {
